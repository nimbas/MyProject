...
<%@ attribute name="data_dateformat" required="false" rtexprvalue="true" %>
...
<c:set value="${data_dateformat}" var="yourDateFormat"/>
...
<c:if test="${empty data_dateformat }"><c:set value="yy-mm-dd" var="yourDateFormat"/></c:if>
<div class="form-group">
	<label class="control-label ${yourLabelClass }" for="prepend">${label }</label>
	<div class="${yourInputClass }">
		<div class="input-group">
			<input type="text" id="${id }" name="${yourName }" placeholder="${placeHolder }" class="form-control datepicker${not empty required and required eq 'required' ? ' required' : ''}" data-dateformat="${yourDateFormat }" value="${yourValue}"	${yourDisabled } ${yourRequired }>
