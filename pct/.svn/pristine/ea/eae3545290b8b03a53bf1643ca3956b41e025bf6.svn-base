...
<%@ attribute name="buttonId" required="false" rtexprvalue="true" %>
...
<c:set value="${buttonId }" var="yourButtonId"/>
<c:set value="${buttonText }" var="yourButtonText"/>
...
<div class="form-group">
	<label class="control-label ${yourLabelClass }" for="${id }">${label }</label>
	<div class="${yourInputClass }"> 
		<div class="input-group">
			...
			<div class="input-group-btn">
				<button class="btn btn-default btn-primary" type="button"  id="${yourButtonId}"><i class="fa fa-search"></i>
				</button>
			</div>
		</div>
	</div>
