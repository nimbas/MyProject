...
<script type="text/javascript">
	...
	var pagefunction = function() {
		...
		if ($("#fill-chart").length) {
			...
			plot_5 = $.plot($("#fill-chart"), dataset, {
		...
		if ($('#pie-chart').length) {
				...
				plot_6 = $.plot($("#pie-chart"), data_pie, {
					series : {
						pie : {
							show : true,
							innerRadius : 0.5,
		...
		if ($('#updating-chart').length) {
			...
			plot_8 = $.plot($("#updating-chart"), [getRandomData()], options);
	
			function update() {

				try {
					plot_8.setData([getRandomData()]);
					// since the axes don't change, we don't need to call plot_7.setupGrid()
					plot_8.draw();
		
					var demointerval = setTimeout(update, updateInterval);
				}
				catch(err) {
	...
	loadScript("${BOOT_JS}/plugin/flot/jquery.flot.cust.min.js", function(){
		loadScript("${BOOT_JS}/plugin/flot/jquery.flot.resize.min.js", function(){
			loadScript("${BOOT_JS}/plugin/flot/jquery.flot.fillbetween.min.js", function(){
				loadScript("${BOOT_JS}/plugin/flot/jquery.flot.orderBar.min.js", function(){
					loadScript("${BOOT_JS}/plugin/flot/jquery.flot.pie.min.js", function(){
						loadScript("${BOOT_JS}/plugin/flot/jquery.flot.time.min.js", function(){
							loadScript("${BOOT_JS}/plugin/flot/jquery.flot.tooltip.min.js", pagefunction);
						});
					});
				});
			});
