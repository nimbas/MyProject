var _console = window.console || {log:function() {}}; // IE8이하일경우 이 코드를 삽입
var isLoggable = true ;
var isWarnLoggable = true ;
var isInfoLoggable = true ;
var isErrorLoggable = true ;
var gf_console = {} ;

gf_console.log = function(message){
	if(isLoggable) _console.log(message);
};
gf_console.warn= function(message){
	if(isWarnLoggable) _console.warn(message);
};
gf_console.info= function(message){
	if(isInfoLoggable) _console.info(message);
};
gf_console.error = function(message){
	if(isErrorLoggable) _console.error(message);
};
gf_console.json= function(){
	var id, jsonObj ;
	var args = arguments.length ;
	if(args == 1){
		if(arguments[0] != null){
			try{
				gf_console.log(JSON.stringify(arguments[0]));
			}catch(e){
				gf_console.log('['+arguments[0]+'] JSON.stringify error !' );
			}
		}else{
			gf_console.log('null');
		}
		...
		if(arguments[1] != null){
			try{
				gf_console.log('['+arguments[0]+']'+JSON.stringify(arguments[1]));	
			}catch(e){
				gf_console.log('['+arguments[0]+'] JSON.stringify error !' );
			}
			
		}else{
			gf_console.log('['+arguments[0]+'] null');
		}
	}
 ...
 *  isASync: 싱크여부
 */
function defaultAjaxCallJson(url, jsonformat,  successMessage, failMessage, redirectFunction, isASync, loadingDiv) {
	if (loadingDiv != "" && loadingDiv != null) {
		divMask(loadingDiv,"loading...");
	}
	$.ajax({
		type : "POST",  
		async:isASync,
...
function gf_openDialog(strdialogDivName, strUrl, strParam, strTitle, intWidth, intHeight, boolModal) {
    gf_console.log(strParam); 
    Dialog.open({
		div : strdialogDivName, 
		url : baseUrl+strUrl+"?dialogName="+strdialogDivName.replace('#', '')+"&"+strParam,  
		title : strTitle, 
		width : intWidth, 
		height : intHeight, 
		modal : boolModal
...
function gf_addGridList(jqGridName, jsonData){
	...
	if(jsonData != "" && jsonData != null && typeof(jsonData.length) != "undefined"){ // json 배열이 한개 넘어올떄
		rowid = parseInt(gf_getMaxIndex(jqGridName));
...
//그리드의 색상을 업데이트함
//gf_paintGridByRowId('#testId', 1, {color:'red'}); 
function gf_paintGridByRowId(jqGridName, rowid, attrJson){
	var selIds = $(jqGridName).jqGrid('getDataIDs') ;
	var retRowId = ''; 
	if (selIds != '' && selIds != null){
		for(var k=0; k < selIds.length; k++){
			if(rowid == selIds[k]){
				...
				$.each(gridJsonData, function(itemName, itemValue){
					 $(jqGridName).jqGrid('setCell',rowid, itemName, '', attrJson);
				});
			}
		}
	}
...
//입력된 name == value인것을 찾아 선택되게한다.
function gf_setSelectByColumnInfo(jqGridName, columnName, columnValue){
	var selIds = $(jqGridName).jqGrid('getDataIDs') ;
	var retRowId = ''; 
	if (selIds != '' && selIds != null){
		for(var k=0; k < selIds.length; k++){
	...
	if(retRowId != ''){
		$(jqGridName).jqGrid('setSelection', retRowId);	
	}	
}
...
function gf_getTodayDate2(){
	...
	//gf_console.log("Date = " + arguments[0]);
	var _year  = _date.getFullYear();
	var _month = "" + (_date.getMonth() + 1);
	var _day   = "" + _date.getDate();
	...
	//gf_console.log("Converted  _date.getHours() = " +  _date.getHours());
	//gf_console.log("Converted  _date.getUTCHours() = " +  _date.getUTCHours());
	//gf_console.log("Converted data = " + tmp);
	return tmp;
}


function gf_getTodayDate3(){
	var argSize = arguments.length ;
	var _date  = new Date();
	var pattern = "/" ;
	if(argSize == 1){
...
function gf_getBeforeMonthDate(){  
	 var args = arguments.length ;
	 var gapDate = 0 ;
	 var pattern = "/" ;
	 if ( args==1){
...
function gf_CheckSttsByEntr(pMsgYn) {
	...
	var v_msg2 = 'Only Active[ACT} Status!!!';
...
function isAuth(authName) {
	...
	$.ajax({
		...
		success : function(data) {
			...
			gf_console.log(resultYn + "..." + result);
...
function disableButton(btnObj){
	...
	$(btnObj).parent().removeClass('btns3').addClass('btns3_1');
}
function enableButton(btnObj){
	$(btnObj).removeAttr('disabled');
	$(btnObj).parent().removeClass('btns3_1').addClass('btns3');
}
...
/*  //최초 그리드 width
    ...
    $(window).bind('resize', function() {
    	var w = $('#'+div_id).width()-2 ;
        $('#' + grid_id).setGridWidth(w,false);
     }).trigger('resize');

}

function gf_getDeviceFtrBySvc(svcCd){
	/*
	707	950	Equipment Deposit
	708	950	Handset. Deposit
	709	950	IPTV(STB) Equip. Deposit
	710	950	Broadband Equip. Deposit
	711	950	VoIP Equip. Deposit*/
