...
public class CodeOutTag extends OutSupport {
	...
	private void init() {
		...
		typeGbn = codeType = option = include = exclude = lang = title = null ;
    ...
    private void evaluateExpressions() throws JspException {
		try {
			...
			value = (String) ExpressionUtil.evalNotNull("list", "value", value_, String.class, this, pageContext);
			...
			//옵션값이 있는 경우 : radio, checkbox, listbox 등 
				...
				List<LookupCode> codes = codeService.getCodeListFromCache(lookupCode);
				if(codes != null){
			...
			Boolean escape2 = ((Boolean) ExpressionUtil.evalNotNull("list", "escapeXml", escapeXml_, Boolean.class, this, pageContext));
			if (escape != null)
	...
	private void makeOptoinString(StringBuffer buffer, int i, LookupCode code, boolean escape) {
		...
		String codeName = escapeXml(code.getMeaning(), escape) ;
		if("listbox".equals(option)){
			...
			.append((StringUtils.hasText(title)) ? " title='" + title +"'" : "")
			.append("/>&nbsp;")
